...
@section('content')
    ...
    <div class="p-6 sm:ml-64 mt-10">
        <div class="bg-white p-8 rounded-lg shadow-lg dark:border-gray-700 dark:bg-gray-900">
            ...
            <form id="bookingForm" action="{{ route('bookings.updateBooking', $booking->id) }}" method="POST"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $booking->room->name }}" name="room_name" id="room_name"
                            class="mt-2 border block w-full rounded-lg border-gray-500 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-md p-3 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $booking->name }}" name="name" id="name"
                            class="mt-2 block w-full rounded-lg border-gray-500 border shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-md p-3 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
                            required>
                    </div>
                    ...
                    <div>
                        <label for="status"
                            class="block text-lg font-medium text-gray-700 dark:text-gray-300">Status</label>
                        <select name="status" id="status"  class="mt-2 block w-full rounded-lg border-gray-500 border shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-md p-3 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600">
                            <option value="Pending" {{ $booking->status === 'Pending' ? 'selected' : '' }}>Pending</option>
                            <option value="Accept" {{ $booking->status === 'Accept' ? 'selected' : '' }}>Accept</option>
                            <option value="Check Out" {{ $booking->status === 'Check Out' ? 'selected' : '' }}>Check Out</option>
